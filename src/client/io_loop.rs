...
pub struct Remote<T: InvokeUiSession> {
    ...
    // Split out from data_count, which counts every message. Video-only figures are what
    // matter when diagnosing lag, and mixing audio/cursor/clipboard into "speed" made the
    // displayed number unusable for that.
    video_bytes: Arc<AtomicUsize>,
    video_frames_in: Arc<AtomicUsize>,
    video_dropped: Arc<AtomicUsize>,
    resync_requests: Arc<AtomicUsize>,
...
impl<T: InvokeUiSession> Remote<T> {
    pub fn new(
        ...
        Self {
            ...
            video_bytes: Arc::new(AtomicUsize::new(0)),
            video_frames_in: Arc::new(AtomicUsize::new(0)),
            video_dropped: Arc::new(AtomicUsize::new(0)),
            resync_requests: Arc::new(AtomicUsize::new(0)),
                ...
                loop {
                    tokio::select! {
                        ...
                        _ = status_timer.tick() => {
                            ...
                            // One line per second, greppable as "rd-stat client".
                            //
                            //   in     frames arriving from the peer
                            //   shown  frames actually handed to the renderer (true display rate)
                            //   dec    decoder throughput
                            //   q      frames waiting to be decoded (each one is added latency)
                            //   vid    video-only throughput; total includes audio/cursor/clipboard
                            //   drop   queue overflows (each punches a hole in the decode chain)
                            //   resync key frames requested to recover from those holes
                            //
                            // shown < in means frames are being discarded or the decoder cannot
                            // keep up; a non-zero drop/resync is the pathological case.
                            {
                                let vbytes = self.video_bytes.swap(0, Ordering::Relaxed);
                                let vin = self.video_frames_in.swap(0, Ordering::Relaxed);
                                let dropped = self.video_dropped.swap(0, Ordering::Relaxed);
                                let resync = self.resync_requests.swap(0, Ordering::Relaxed);
                                let shown: i32 = fps.values().sum();
                                let qlen: usize = self
                                    .video_threads
                                    .values()
                                    .map(|v| v.video_queue.read().unwrap().len())
                                    .sum();
                                let dec: usize = self
                                    .video_threads
                                    .values()
                                    .filter_map(|v| *v.decode_fps.read().unwrap())
                                    .sum();
                                let per_s = |n: usize| (n as u128 * 1000 / elapsed.max(1)) as usize;
                                if vin + dropped + resync > 0 {
                                log::info!(
                                    "rd-stat client: in={}/s shown={}/s dec={}/s q={} vid={:.0}kbps total={} drop={} resync={} direct={}",
                                    per_s(vin),
                                    shown,
                                    dec,
                                    qlen,
                                    (per_s(vbytes) * 8) as f32 / 1000.0,
                                    speed,
                                    dropped,
                                    resync,
                                    direct,
    ...
    // Report what this machine can actually decode, and keep the local backlog guard.
    //
    // The server detects a slow *link* on its own by watching how long its writes block.
    // It cannot detect a slow *decoder*: we keep draining the socket promptly, so its
    // writes never block, and the excess piles up here as display latency instead. This
    // report is the only way that shows up on the server.
    //
    // Unlike the original implementation this reports the raw decode rate and releases
    // the cap in a single step. Deriving a fraction of the decode rate and then crawling
    // back upwards is what used to leave sessions pinned well below what the link could
    // carry; the server applies its own smoothing, so damping it twice only hurts.
    #[inline]
    fn fps_control(&mut self, _direct: bool, real_fps_map: HashMap<usize, i32>) {
        ...
        // A queue that keeps refilling means the decoder, not the link, is the limit.
        // A couple of frames in flight is normal jitter, so only react past that.
        let backlogged = self
            .video_threads
            .values()
            .any(|t| t.video_queue.read().unwrap().len() > std::cmp::max(2, min_decode_fps / 4));
        let report = if backlogged {
            (min_decode_fps as i32).clamp(1, client::FIXED_CUSTOM_FPS) as usize
        } else {
            client::FIXED_CUSTOM_FPS as usize
        };
        if self.handler.lc.read().unwrap().last_auto_fps != Some(report) {
            let mut misc = Misc::new();
            misc.set_auto_adjust_fps(report as _);
            let mut msg = Message::new();
            msg.set_misc(misc);
            self.sender.send(Data::Message(msg)).ok();
            log::info!("Report decode capability {} fps", report);
            self.handler.lc.write().unwrap().last_auto_fps = Some(report);
        }
        ...
        if let Ok(msg_in) = Message::parse_from_bytes(&data) {
            match msg_in.union {
                Some(message::Union::VideoFrame(vf)) => {
                    ...
                    self.video_frames_in.fetch_add(1, Ordering::Relaxed);
                    self.video_bytes
                        .fetch_add(vf.compute_size() as usize, Ordering::Relaxed);
                    if Self::contains_key_frame(&vf) {
                        ...
                        if video_queue.force_push(vf).is_some() {
                            // Overflow. force_push evicted the oldest frame, so the decode
                            // chain now has a hole and everything queued behind it would
                            // decode to garbage. Ask for a key frame to start cleanly.
                            drop(video_queue);
