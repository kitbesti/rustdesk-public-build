...
// Frames the client will buffer before it starts discarding.
//
// This is a latency budget, not a capacity: every queued frame is one frame of delay
// between something happening on the remote screen and it appearing here. At 120 frames
// it held a full second at 120fps, so a client that fell slightly behind accumulated a
// second of lag before anything reacted - and then overflowed, which used to trigger a
// full pipeline restart. 30 frames is ~0.25s at the interactive target.
pub const VIDEO_QUEUE_SIZE: usize = 30;
// Rate the client asks for on an interactive session. Matches the controlled side's
// interactive target; the client lowers it via `auto_adjust_fps` only when its decoder
// actually falls behind.
pub const FIXED_CUSTOM_FPS: i32 = 120;
...
pub struct LoginConfigHandler {
    ...
    // Last decode-capability figure reported to the peer, so we only send on change.
...
impl LoginConfigHandler {
    fn forced_custom_fps(&self) -> Option<i32> {
        if self.conn_type.eq(&ConnType::DEFAULT_CONN) || self.conn_type.eq(&ConnType::VIEW_CAMERA) {
            Some(FIXED_CUSTOM_FPS)
        } else {
            None
        }
    }
    ...
    fn get_option_message(&self, ignore_default: bool) -> Option<OptionMessage> {
        ...
        if let Some(fps) = self.forced_custom_fps() {
            msg.custom_fps = fps;
    ...
    pub fn set_custom_fps(&mut self, fps: i32, save_config: bool) -> Message {
        let fps = self.forced_custom_fps().unwrap_or(fps);
        let mut misc = Misc::new();
        misc.set_option(OptionMessage {
            custom_fps: fps,
            ..Default::default()
