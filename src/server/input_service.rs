...
// Presses and scrolls injected so far; the capturer is told when this moves.
pub static PRESSES: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
...
pub fn handle_mouse_simulation_(evt: &MouseEvent, conn: i32) {
    ...
    let buttons = evt.mask >> 3;
    let evt_type = evt.mask & MOUSE_TYPE_MASK;
    if matches!(evt_type, MOUSE_TYPE_DOWN | MOUSE_TYPE_WHEEL | MOUSE_TYPE_TRACKPAD) {
