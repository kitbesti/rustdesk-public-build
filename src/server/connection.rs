...
pub struct Connection {
    ...
    // Frames written to an ack-required (web) peer that it has not yet confirmed.
    //
    // A WebSocket send completes into a userspace buffer, so a backed-up web client
    // never makes our write block the way a raw TCP peer does. Its explicit
    // `VideoReceived` acks are the only backpressure signal available for it.
    video_unacked: usize,
...
// A video write that takes longer than this means the socket would not accept the
// frame, i.e. the peer is not draining what we already sent.
const VIDEO_SEND_CONGESTED_MS: u128 = 120;
// Backlog past which we stop hoping the peer catches up on its own and resynchronise
// it on a key frame instead. ~0.5s of video at the interactive target.
const VIDEO_BACKLOG_RESYNC: usize = 30;
// Queued frames that mean this peer is falling behind. One or two is ordinary
// scheduling jitter; three means we are not keeping up with the encoder.
const VIDEO_BACKLOG_CONGESTED: usize = 3;
// Unconfirmed frames past which an ack-required peer counts as congested.
const VIDEO_UNACKED_CONGESTED: usize = 4;

// Per-peer pacing.
//
// One encoder serves every viewer at the fastest peer's rate, so a peer that asked for
// less receives more than it wants. Its frames cannot simply be decimated - dropping one
// mid-GOP leaves its decoder predicting from a picture it never got - so the only safe
// skip is forward to a key frame. This connection therefore tracks how far ahead of its
// peer's requested rate it has run, and once that exceeds a whole GOP's worth of surplus
// it asks for a key frame and cuts to it. Bounded staleness, intact decoding, and the
// peer's report finally does something instead of being collected and discarded.
const VIDEO_PACING_SURPLUS_FRAMES: f32 = 24.0;

#[derive(Default)]
struct PendingVideo {
    switch_display: Option<Arc<Message>>,
    // Frames in send order. Order matters: every frame except a key frame is coded
    // as a delta against its predecessor.
    frames: VecDeque<Arc<Message>>,
}

impl PendingVideo {
    fn push(&mut self, msg: Arc<Message>) {
        match &msg.union {
            Some(message::Union::VideoFrame(_)) => {
                self.frames.push_back(msg);
            }
            Some(message::Union::Misc(misc)) => {
                if let Some(misc::Union::SwitchDisplay(_)) = &misc.union {
                    // Anything queued for the old display is meaningless now.
                    self.switch_display = Some(msg);
                    self.frames.clear();
                }
            }
            _ => {}
    ...
    // Discard the backlog only as far as a key frame, and only once it is clearly
    // beyond recovery.
    //
    // Skipping an inter-coded frame leaves the peer's decoder predicting from a
    // picture we never sent, so everything after it decodes into garbage until the
    // next key frame arrives. That is why frames must never be dropped merely
    // because newer ones exist: a key frame is the only safe place to cut.
    // Cut to the newest key frame regardless of how deep the backlog is. Used by pacing,
    // where the trigger is "this peer is ahead of the rate it asked for", not queue depth.
    fn drop_stale_to_key_frame_now(&mut self) -> usize {
        let Some(cut) = self.frames.iter().rposition(|m| contains_key_frame(m)) else {
            return 0;
        };
        if cut == 0 {
            return 0;
    ...
    fn drop_stale_to_key_frame(&mut self) -> usize {
        if self.frames.len() <= VIDEO_BACKLOG_RESYNC {
            return 0;
        }
        let Some(cut) = self.frames.iter().rposition(|msg| contains_key_frame(msg)) else {
            // No resync point available; keep the backlog intact and let the source
            // rate back off. Corrupt video is worse than late video.
            return 0;
        };
        if cut == 0 {
            return 0;
...
#[inline]
fn contains_key_frame(msg: &Message) -> bool {
...
// The single message a send-loop turn should write, if any.
enum VideoTurn {
    SwitchDisplay(Arc<Message>),
    Frame(Arc<Message>),
    Idle,
}
...
impl PendingVideo {
    fn is_empty(&self) -> bool {
        self.switch_display.is_none() && self.frames.is_empty()
    }

    // Take the one message to write this turn.
    //
    // Exactly one, never more: the loop that calls this is also the only reader of the
    // peer's input, and a write blocks until the socket accepts it. Draining the whole
    // backlog in one turn would leave mouse and keyboard events unread for the sum of
    // those writes, which is what made operations appear to be ignored on a poor link.
    // One per turn bounds that wait to a single write no matter how deep the backlog.
    fn take_turn(&mut self) -> VideoTurn {
        if let Some(msg) = self.switch_display.take() {
            return VideoTurn::SwitchDisplay(msg);
        }
        match self.frames.pop_front() {
            Some(msg) => VideoTurn::Frame(msg),
            None => VideoTurn::Idle,
        }
    }
}
...
impl Connection {
    // Wake the video branch when there is something to write.
    //
    // Returns immediately when work is already queued from a previous turn, so the
    // backlog keeps draining one frame at a time without waiting for the encoder to
    // produce another. Otherwise it waits on the channel, which is cancel-safe.
    async fn next_video(
        ...
        match rx_video.recv().await {
            Some((_, msg)) => Some(msg),
            // Channel closed with nothing queued: park forever so this branch stops
            // waking the select rather than spinning on an immediately-ready `None`.
    ...
    pub async fn start(
        ...
        let mut last_congestion_report = Instant::now();
        // Video queued for this peer, carried across select turns so the loop can write
        // one frame at a time and keep servicing input in between.
        let mut pending_video = PendingVideo::default();
        // Frames sent to this peer beyond what its own requested rate would allow.
        let mut pacing_surplus: f32 = 0.0;
        let mut pacing_instant = Instant::now();
        // Per-second send-side counters, reported alongside the client's own line so the
        // two ends can be compared directly.
        let mut stat_instant = Instant::now();
        let (mut stat_frames, mut stat_bytes, mut stat_dropped, mut stat_blocked) =
        ...
        loop {
            tokio::select! {
                // Input first, unconditionally.
                //
                // `biased` polls branches in written order and takes the first that is
                // ready, so putting the peer's inbound messages at the top means keyboard
                // and mouse are always drained before any video is written. Upstream once
                // tried `biased` with *video* first and reverted it; this is the opposite
                // ordering and the one the priority model calls for - operations must
                // never wait behind pixels.
                //
                // This cannot starve the other branches in practice: input is orders of
                // magnitude lower rate than the loop can service, and a branch is only
                // taken when it is already ready.
                biased;
                ...
                new_frame = Self::next_video(&mut rx_video, &pending_video) => {
                    if let Some(value) = new_frame {
                        pending_video.push(value);
                    }
                    // Take everything else already queued so the backlog can be judged as
                    // a whole before deciding what to write.
                    while let Ok((_, msg)) = rx_video.try_recv() {
                        pending_video.push(msg);
                    }
                    // Charge this peer for the frames it is about to receive against the
                    // rate it actually asked for.
                    {
                        let elapsed = pacing_instant.elapsed().as_secs_f32();
                        pacing_instant = Instant::now();
                        let allowed = video_service::VIDEO_QOS
                            .lock()
                            .unwrap()
                            .user_target_fps(id) as f32
                            * elapsed;
                        pacing_surplus += pending_video.frames.len() as f32 - allowed;
                        pacing_surplus = pacing_surplus.clamp(0.0, VIDEO_PACING_SURPLUS_FRAMES * 4.0);
                    }
                    if pacing_surplus >= VIDEO_PACING_SURPLUS_FRAMES {
                        // Far enough ahead of what this peer wants to be worth resetting.
                        // Cut to a key frame if one is already queued, otherwise ask for
                        // one and keep delivering until it arrives - a gap mid-GOP would
                        // cost this peer more than the surplus does.
                        if pending_video.drop_stale_to_key_frame_now() > 0 {
                            pacing_surplus = 0.0;
                        } else {
                            video_service::request_key_frame();
                        }
                    }
                    let dropped = pending_video.drop_stale_to_key_frame();
                    stat_dropped += dropped;
                    if dropped > 0 {
                        log::info!("conn {id}: video backlog too deep, resynced on a key frame, dropped {dropped} frames");
                    } else if pending_video.frames.len() > VIDEO_BACKLOG_RESYNC {
                        // Too far behind to be worth delivering, but there is no key frame
                        // in the backlog to cut to - with a multi-second GOP that is the
                        // normal case. Ask for one so a cut point arrives shortly; until it
                        // does the backlog is delivered intact, because a gap mid-GOP would
                        // corrupt the peer's decoder for longer than the delay costs.
                        video_service::request_key_frame();
                    }
                    // A backlog is itself evidence the peer is not keeping up, before we
                    // even attempt a write. Check `dropped` too: resyncing on a key frame
                    // leaves few frames queued, which would otherwise read as healthy
                    // precisely when things are at their worst.
                    let mut congested =
                        dropped > 0 || pending_video.frames.len() >= VIDEO_BACKLOG_CONGESTED;
                    if conn.video_ack_required && conn.video_unacked >= VIDEO_UNACKED_CONGESTED {
                        congested = true;
                    }

                    // Exactly one message per turn, then back to the select so the peer's
                    // input gets read. See `PendingVideo::take_turn`.
                    let mut closed = None;
                    match pending_video.take_turn() {
                        VideoTurn::SwitchDisplay(value) => {
                            if let Err(err) = conn.stream.send(value.as_ref()).await {
                                closed = Some(err.to_string());
                            }
                        }
                        VideoTurn::Frame(value) => {
                            if conn.video_ack_required {
                                conn.video_unacked += 1;
                            }
                            let began = Instant::now();
                            let nbytes = value.compute_size() as usize;
                            if let Err(err) = conn.stream.send(value.as_ref()).await {
                                closed = Some(err.to_string());
                            } else if { stat_frames += 1; stat_bytes += nbytes;
                                        began.elapsed().as_millis() > VIDEO_SEND_CONGESTED_MS } {
                                stat_blocked += 1;
                                // Local fact, used only to decide whether this connection is
                                // keeping up enough to be worth queueing for - never fed
                                // back as a claim about the network.
                                congested = true;
                            }
                        }
                        VideoTurn::Idle => {}
                    ...
                    if congested && last_congestion_report.elapsed() >= Duration::from_secs(5) {
                        last_congestion_report = Instant::now();
                        log::debug!("conn {id}: video send is not keeping up");
                    }
                    // One line per second, greppable as "rd-stat server".
                    //
                    //   sent    frames actually written to this peer's socket
                    //   kbps    bytes those frames occupied - real throughput, not a target
                    ...
                    //
                    // Compare sent/s here with in/s on the client: a gap is loss or backlog
                    // in transit, while sent < target means the capturer is the limit.
                    {
                        let ms = stat_instant.elapsed().as_millis();
                        if ms >= 1000 {
                            stat_instant = Instant::now();
                            let per_s = |n: usize| (n as u128 * 1000 / ms.max(1)) as usize;
                            let (target, bitrate) = {
                                let q = video_service::VIDEO_QOS.lock().unwrap();
                                (q.fps(), q.bitrate())
                            };
                            if stat_frames + stat_dropped + stat_blocked > 0 {
                            log::info!(
                                "rd-stat server conn {id}: sent={}/s {:.0}kbps target={}fps cfg={}kbps q={} drop={} slow={}",
                                per_s(stat_frames),
                                (per_s(stat_bytes) * 8) as f32 / 1000.0,
                                target,
                                bitrate,
                                pending_video.frames.len(),
                                stat_dropped,
                                stat_blocked,
                            );
                            }
                            stat_frames = 0; stat_bytes = 0; stat_dropped = 0; stat_blocked = 0;
                        }
                    }
                    if let Some(err) = closed {
                        conn.on_close(&err, false).await;
                        break;
                    }
                },
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
            ...
            match msg.union {
                ...
                Some(message::Union::Misc(misc)) => match misc.union {
                    ...
                    Some(misc::Union::RefreshVideoDisplay(_display)) => {
                        if self.should_handle_render_broadcast_message() {
                            self.refresh_video_keyframe();
                            self.update_auto_disconnect_timer();
                        }
                    }
                    Some(misc::Union::VideoReceived(_)) => {
                        self.video_unacked = self.video_unacked.saturating_sub(1);
                    }
    ...
    // Recover a peer whose decoder has lost sync, without disturbing anyone else.
    //
    // The heavy `refresh_video_display` path sets OPTION_REFRESH, which makes the capture
    // loop `bail!("SWITCH")` and rebuild the capturer and encoder from scratch. That costs
    // hundreds of milliseconds, and because one encoder feeds every subscriber it freezes
    // *all* of them - so a single peer overflowing its queue stalled the whole session.
    // A key frame restores that peer completely and costs one frame.
    fn refresh_video_keyframe(&self) {
        video_service::request_key_frame();
    }
...
mod video_send_path {
    ...
    fn switch_display() -> Arc<Message> {
        let mut misc = Misc::new();
        misc.set_switch_display(SwitchDisplay::default());
        let mut msg = Message::new();
        msg.set_misc(misc);
        Arc::new(msg)
    }

    fn queue(spec: &[bool]) -> PendingVideo {
        let mut p = PendingVideo::default();
        for key in spec {
            p.push(frame(*key));
        }
        p
    ...
    #[test]
    fn key_frames_are_recognised() {
        assert!(contains_key_frame(&frame(true)));
        assert!(!contains_key_frame(&frame(false)));
        assert!(!contains_key_frame(&switch_display()));
    }

    // The bug this replaced: dropping an inter-coded frame leaves the peer's decoder
    // predicting from a picture it never received. A tolerable backlog must be delivered
    // whole, however tempting the newest frame looks.
    #[test]
    fn a_tolerable_backlog_is_never_thinned() {
        let mut pending = queue(&[true, false, false, false, false]);
        assert_eq!(pending.drop_stale_to_key_frame(), 0);
        assert_eq!(pending.frames.len(), 5);
    }

    #[test]
    fn a_deep_backlog_resyncs_only_at_a_key_frame() {
        let mut spec = vec![false; VIDEO_BACKLOG_RESYNC + 10];
        spec[0] = true;
        // The only safe cut point, deliberately not the newest frame.
        let key_at = VIDEO_BACKLOG_RESYNC / 2;
        spec[key_at] = true;
        let mut pending = queue(&spec);

        let dropped = pending.drop_stale_to_key_frame();

        assert_eq!(dropped, key_at, "must cut exactly to the newest key frame");
        assert!(
            contains_key_frame(&pending.frames[0]),
            "what remains must start on a key frame the decoder can resume from"
        );
        assert_eq!(pending.frames.len(), spec.len() - key_at);
    }

    // Without a resync point, delivering late is the only correct option: a decoder fed
    // a gap produces garbage until the next key frame, which is worse than lag.
    #[test]
    fn a_deep_backlog_with_no_key_frame_is_kept_intact() {
        let mut pending = queue(&vec![false; VIDEO_BACKLOG_RESYNC + 10]);
        assert_eq!(pending.drop_stale_to_key_frame(), 0);
        assert_eq!(pending.frames.len(), VIDEO_BACKLOG_RESYNC + 10);
    }

    #[test]
    fn switching_display_discards_frames_for_the_old_one() {
        let mut pending = queue(&[true, false, false]);
        pending.push(switch_display());
        assert!(pending.frames.is_empty());
        assert!(!pending.is_empty(), "the switch itself is still pending");
        assert!(matches!(pending.take_turn(), VideoTurn::SwitchDisplay(_)));
        assert!(pending.is_empty());
    }

    // The input-delivery guarantee. The send loop is also the peer's only reader, so
    // every message it writes must cost exactly one turn; the select regains control
    // between turns and can service input. A backlog therefore delays input by one
    // write, not by the whole backlog.
    #[test]
    fn a_backlog_costs_one_turn_per_message_so_input_is_never_starved() {
        let depth = VIDEO_BACKLOG_RESYNC - 1;
        let mut pending = queue(&vec![false; depth]);

        let mut turns = 0;
        while !pending.is_empty() {
            match pending.take_turn() {
                VideoTurn::Idle => panic!("a non-empty queue must yield work"),
                _ => turns += 1,
            }
            assert!(
                turns <= depth,
                "a turn must never write more than one message"
            );
        }

        assert_eq!(
            turns, depth,
            "draining {depth} frames must take {depth} turns, one input opportunity each"
        );
        assert!(matches!(pending.take_turn(), VideoTurn::Idle));
    }

    // --- edge cases around the resync cut ---

    #[test]
    fn a_backlog_of_exactly_the_threshold_is_not_cut() {
        // Boundary: the rule is "past" the threshold, not "at" it.
        let mut spec = vec![false; VIDEO_BACKLOG_RESYNC];
        spec[0] = true;
        let mut pending = queue(&spec);
        assert_eq!(pending.drop_stale_to_key_frame(), 0);
        assert_eq!(pending.frames.len(), VIDEO_BACKLOG_RESYNC);
    }

    #[test]
    fn cutting_prefers_the_newest_key_frame_not_the_first() {
        let mut spec = vec![false; VIDEO_BACKLOG_RESYNC + 20];
        spec[1] = true;
        spec[5] = true;
        let newest = VIDEO_BACKLOG_RESYNC + 10;
        spec[newest] = true;
        let mut pending = queue(&spec);

        assert_eq!(pending.drop_stale_to_key_frame(), newest);
        assert!(contains_key_frame(&pending.frames[0]));
        assert_eq!(pending.frames.len(), spec.len() - newest);
    }

    #[test]
    fn a_key_frame_already_at_the_front_is_not_re_cut() {
        // cut == 0 must be a no-op rather than draining nothing in a loop.
        let mut spec = vec![false; VIDEO_BACKLOG_RESYNC + 5];
        spec[0] = true;
        let mut pending = queue(&spec);
        assert_eq!(pending.drop_stale_to_key_frame(), 0);
        assert_eq!(pending.frames.len(), spec.len());
    }

    #[test]
    fn the_newest_frame_being_a_key_frame_drops_everything_before_it() {
        let mut spec = vec![false; VIDEO_BACKLOG_RESYNC + 3];
        let last = spec.len() - 1;
        spec[last] = true;
        let mut pending = queue(&spec);

        assert_eq!(pending.drop_stale_to_key_frame(), last);
        assert_eq!(pending.frames.len(), 1, "only the key frame should remain");
        assert!(contains_key_frame(&pending.frames[0]));
    }

    #[test]
    fn repeated_cuts_converge_and_never_underflow() {
        // Draining a deep backlog must terminate, whatever the key-frame spacing.
        for spacing in [1usize, 2, 7, VIDEO_BACKLOG_RESYNC + 1] {
            let spec: Vec<bool> = (0..VIDEO_BACKLOG_RESYNC * 3)
                .map(|i| i % spacing == 0)
                .collect();
            let mut pending = queue(&spec);
            let mut guard = 0;
            while !pending.is_empty() {
                pending.drop_stale_to_key_frame();
                assert!(matches!(pending.take_turn(), VideoTurn::Frame(_)));
                guard += 1;
                assert!(guard <= spec.len(), "spacing {spacing} failed to converge");
            }
        }
    }

    #[test]
    fn an_empty_queue_is_safe_to_cut_and_take() {
        let mut pending = PendingVideo::default();
        assert_eq!(pending.drop_stale_to_key_frame(), 0);
        assert!(matches!(pending.take_turn(), VideoTurn::Idle));
        assert!(pending.is_empty());
    }
    ...
    #[test]
    fn a_switch_display_arriving_mid_backlog_discards_the_old_display() {
        let mut pending = queue(&vec![false; VIDEO_BACKLOG_RESYNC + 5]);
        pending.push(switch_display());
        assert!(
            pending.frames.is_empty(),
            "stale frames must not outlive the switch"
        );
        // and frames for the new display queue behind it normally
        pending.push(frame(true));
        assert!(matches!(pending.take_turn(), VideoTurn::SwitchDisplay(_)));
        assert!(matches!(pending.take_turn(), VideoTurn::Frame(_)));
        assert!(matches!(pending.take_turn(), VideoTurn::Idle));
    }
    ...
    #[test]
    fn non_video_messages_never_enter_the_video_queue() {
        let mut pending = PendingVideo::default();
        let mut misc = Misc::new();
        misc.set_refresh_video(true);
        let mut msg = Message::new();
        msg.set_misc(misc);
        pending.push(Arc::new(msg));
        assert!(
            pending.is_empty(),
            "only video and display switches belong here"
        );
    }

    // --- decoder resync must be cheap, not a pipeline restart ---
    //
    // A peer whose queue overflows asks for a refresh. That used to set OPTION_REFRESH,
    // making the capture loop bail!("SWITCH") and rebuild the capturer and encoder - which
    // freezes every other viewer too, because one encoder feeds them all. On a fast link
    // the client queue fills quickly, so this fired repeatedly and the session spent its
    // time restarting. These pin the cheap path in place.

    #[test]
    fn a_key_frame_alone_restores_a_desynced_decoder() {
        // A key frame is self-contained: a decoder with no history can start from it.
        let mut pending = PendingVideo::default();
        pending.push(frame(true));
        match pending.take_turn() {
            VideoTurn::Frame(m) => assert!(contains_key_frame(&m)),
            _ => panic!("expected a frame"),
        }
    }

    #[test]
    fn resync_requests_are_idempotent() {
        // Several peers overflowing at once must not queue up several restarts; the
        // request is a flag, so repeated asks collapse into one key frame.
        video_service::request_key_frame();
        video_service::request_key_frame();
        video_service::request_key_frame();
        // Nothing to assert beyond "does not panic or accumulate"; the flag is consumed
        // once by the capture loop.
    }

    // The client budget is a latency budget. Guard the intent so it is not quietly raised
    // back to a value that buffers a second of video.
    #[test]
    fn the_client_queue_is_a_latency_budget() {
        let secs_at_target =
            crate::client::VIDEO_QUEUE_SIZE as f32 / super::super::video_qos::FPS as f32;
        assert!(
            secs_at_target <= 0.35,
            "queue holds {secs_at_target:.2}s at {}fps - too much interactive lag",
            super::super::video_qos::FPS
        );
        assert!(
            crate::client::VIDEO_QUEUE_SIZE >= VIDEO_BACKLOG_CONGESTED * 2,
            "queue must still absorb ordinary jitter"
        );
    }

    // --- per-peer pacing cuts ---
    //
    // A peer that asked for a lower rate than the shared encoder produces must be able to
    // skip ahead, but only at a key frame: decimating an inter-coded stream leaves its
    // decoder predicting from a picture it never received.

    #[test]
    fn pacing_cut_ignores_backlog_depth() {
        // Unlike the backlog cut, this one fires on rate, so it must work on a short queue.
        let mut pending = queue(&[false, false, true, false]);
        assert_eq!(
            pending.drop_stale_to_key_frame(),
            0,
            "too short for the backlog cut"
        );
        assert_eq!(
            pending.drop_stale_to_key_frame_now(),
            2,
            "pacing cut still applies"
        );
        assert!(contains_key_frame(&pending.frames[0]));
    ...
    #[test]
    fn pacing_cut_takes_the_newest_key_frame() {
        let mut pending = queue(&[true, false, true, false, false]);
        assert_eq!(pending.drop_stale_to_key_frame_now(), 2);
        assert!(contains_key_frame(&pending.frames[0]));
        assert_eq!(pending.frames.len(), 3);
    }

    #[test]
    fn pacing_cut_refuses_when_there_is_no_key_frame() {
        // Must deliver intact rather than punch a hole; the caller asks for a key frame.
        let mut pending = queue(&[false, false, false, false]);
        assert_eq!(pending.drop_stale_to_key_frame_now(), 0);
        assert_eq!(pending.frames.len(), 4);
    }

    #[test]
    fn pacing_cut_is_a_no_op_when_already_at_a_key_frame() {
        let mut pending = queue(&[true, false, false]);
        assert_eq!(pending.drop_stale_to_key_frame_now(), 0);
        assert_eq!(pending.frames.len(), 3);
    }
    ...
    #[test]
    fn pacing_cut_on_an_empty_queue_is_safe() {
        let mut pending = PendingVideo::default();
        assert_eq!(pending.drop_stale_to_key_frame_now(), 0);
        assert!(pending.is_empty());
    }

    // The surplus accounting must converge rather than run away in either direction.
    #[test]
    fn pacing_surplus_arithmetic_is_bounded() {
        let cap = VIDEO_PACING_SURPLUS_FRAMES * 4.0;
        let mut surplus: f32 = 0.0;
        // Encoder at 120/s, peer asked for 15/s, one second at a time.
        for _ in 0..100 {
            surplus += 120.0 - 15.0;
            surplus = surplus.clamp(0.0, cap);
            if surplus >= VIDEO_PACING_SURPLUS_FRAMES {
                surplus = 0.0; // a cut succeeded
            }
        }
        assert!(surplus <= cap && surplus >= 0.0);

        // Peer asking for more than it receives must not accumulate negative credit.
        let mut surplus: f32 = 0.0;
        for _ in 0..100 {
            surplus += 10.0 - 120.0;
            surplus = surplus.clamp(0.0, cap);
        }
        assert_eq!(surplus, 0.0, "an under-served peer must not bank credit");
    }

    #[test]
    fn switch_display_is_written_before_the_frames_that_follow_it() {
        let mut pending = PendingVideo::default();
        pending.push(switch_display());
        pending.push(frame(true));
        pending.push(frame(false));
