...
pub struct VpxEncoder {
    ...
    // Set by `request_key_frame`, consumed by the next `encode`.
                ...
                */
                ...
                if config.codec == VpxVideoCodecId::VP9 {
                    ...
                    */
                    ...
                    // A desktop, not a film: text and flat colour, still for stretches,
                    // then whole regions replaced at once. The screen tuning spends its
                    // bits on edges and skips blocks that did not move; cyclic refresh
                    // spreads the cost of a big change over the following frames instead
                    // of one oversized one, which is what a fixed-rate link wants.
                    call_vpx!(vpx_codec_control_(
                        &mut ctx,
                        VP9E_SET_TUNE_CONTENT as _,
                        vp9e_tune_content::VP9E_CONTENT_SCREEN as c_int
                    ));
                    call_vpx!(vpx_codec_control_(&mut ctx, VP9E_SET_AQ_MODE as _, 3 as c_int));
...
impl VpxEncoder {
    pub fn encode<'a>(
        ...
        // `vpx_enc_frame_flags_t` is a C `long`: 64-bit on unix, 32-bit on Windows. Cast
        // to the bindgen typedef rather than a fixed width, or this fails to compile on
        // one platform or the other.
        let flags: vpx_enc_frame_flags_t = if self.force_key_frame {
