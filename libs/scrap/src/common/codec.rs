...
pub trait EncoderApi {
    ...
    /// Ask for the next encoded frame to be a key frame.
    ///
    /// A connection that has fallen behind can only discard queued video at a key frame -
    /// cutting anywhere else leaves the peer's decoder predicting from a picture it never
    /// received. With a multi-second GOP there is usually no such cut point available, so
    /// the backlog has to be requested rather than waited for.
    ///
    /// Default is a no-op: encoders that cannot honour it simply keep their own schedule,
    /// and the caller falls back to delivering the backlog intact.
...
// "Best" is meant for a link with room to spare (tens of Mbit/s): about 12 Mbit/s at
// 1080p and 30 Mbit/s at 4K before the frame-rate scaling in video_qos, which is where
// screen content stops visibly improving. A still screen costs nothing at any setting,
// and "Custom" goes far beyond this either way.
pub const BR_BEST: f32 = 6.0;
