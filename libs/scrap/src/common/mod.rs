...
pub trait TraitCapturer {
    ...
    // Produce a frame on the next call even if the screen has not changed. Capturers that
    // skip unchanged screens override this; the rest have nothing to do.
    fn force_next_frame(&mut self) {}

    // A key or button was just pressed on the captured screen. Capturers that read only
    // what windows report use it to catch what the shell drew in response.
    fn input_hint(&mut self) {}
